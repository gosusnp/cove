...
func newTestProgramExerciseService(t *testing.T) programExerciseFixture {
	...
	p, err := pSvc.Create(ctx, "Test Program", nil, false)
	if err != nil {
		t.Fatal(err)
	}

	ps, err := pSvc.CreateSet(ctx, p.ID, nil, 1, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	return programExerciseFixture{
		svc:        NewProgramExerciseService(pSvc),
		ctx:        ctx,
		programID:  p.ID,
		setID:      ps.ID,
		exerciseID: e.ID,
...
func TestProgramExerciseService_List(t *testing.T) {
	t.Run("returns all exercises for set", func(t *testing.T) {
		...
		_, _ = f.svc.Create(f.ctx, f.programID, f.setID, f.exerciseID, nil, nil, nil, nil, nil)

		list, err := f.svc.List(f.ctx, f.programID, f.setID)
		if err != nil {
...
func TestProgramExerciseService_Get(t *testing.T) {
	t.Run("found", func(t *testing.T) {
		f := newTestProgramExerciseService(t)
		created, _ := f.svc.Create(f.ctx, f.programID, f.setID, f.exerciseID, nil, nil, nil, nil, nil)

		got, err := f.svc.Get(f.ctx, f.programID, f.setID, created.ID)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("not found", func(t *testing.T) {
		f := newTestProgramExerciseService(t)
		_, err := f.svc.Get(f.ctx, f.programID, f.setID, 999)
...
func TestProgramExerciseService_Create(t *testing.T) {
	t.Run("invalid exercise_id returns ValidationError", func(t *testing.T) {
		...
		_, err := f.svc.Create(f.ctx, f.programID, f.setID, 0, nil, nil, nil, nil, nil)
	...
	t.Run("success", func(t *testing.T) {
		f := newTestProgramExerciseService(t)
		pe, err := f.svc.Create(f.ctx, f.programID, f.setID, f.exerciseID, nil, nil, nil, nil, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestProgramExerciseService_Update(t *testing.T) {
	t.Run("invalid exercise_id returns ValidationError", func(t *testing.T) {
		f := newTestProgramExerciseService(t)
		pe, _ := f.svc.Create(f.ctx, f.programID, f.setID, f.exerciseID, nil, nil, nil, nil, nil)

		_, err := f.svc.Update(f.ctx, f.programID, f.setID, pe.ID, 0, nil, nil, nil, nil, nil)
		var ve *ValidationError
		if !errors.As(err, &ve) {
			t.Fatalf("got %v, want ValidationError", err)
		}
	...
	t.Run("not found returns ErrNotFound", func(t *testing.T) {
		...
		_, err := f.svc.Update(f.ctx, f.programID, f.setID, 999, f.exerciseID, nil, nil, nil, nil, nil)
		if !errors.Is(err, ErrNotFound) {
			t.Errorf("got %v, want ErrNotFound", err)
		}
	})
...
func TestProgramExerciseService_Delete(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		if err := f.svc.Delete(f.ctx, f.programID, f.setID, created.ID); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		_, err := f.svc.Get(f.ctx, f.programID, f.setID, created.ID)
		if !errors.Is(err, ErrNotFound) {
	...
	t.Run("not found returns ErrNotFound", func(t *testing.T) {
		...
		err := f.svc.Delete(f.ctx, f.programID, f.setID, 999)
		if !errors.Is(err, ErrNotFound) {
			t.Errorf("got %v, want ErrNotFound", err)
		}
	})
