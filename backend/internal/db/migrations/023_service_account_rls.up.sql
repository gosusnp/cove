...
-- Extend SELECT and UPDATE RLS policies on all tenant-owned tables to allow
-- service accounts through. INSERT and DELETE policies are unchanged — no
-- service account use case exists for those operations.
--
-- Service account identity is signalled by app.current_is_service = 'true'
-- (set by NewServiceScopedQuerier); tenant isolation falls back to explicit
-- org_id parameters at the store level.
...
USING (org_id = cove.current_app_org_id() OR cove.current_app_is_service());
...
USING (org_id = cove.current_app_org_id() OR cove.current_app_is_service())
WITH CHECK (org_id = cove.current_app_org_id() OR cove.current_app_is_service());

-- exercises
DROP POLICY exercises_select ON cove.exercises;
CREATE POLICY exercises_select ON cove.exercises
FOR SELECT TO PUBLIC
USING (is_public = true OR org_id = cove.current_app_org_id() OR cove.current_app_is_service());
...
FOR SELECT TO PUBLIC
USING (org_id = cove.current_app_org_id() OR is_public = true OR cove.current_app_is_service());
...
-- program_versions (append-only: SELECT only; no UPDATE policy exists)
